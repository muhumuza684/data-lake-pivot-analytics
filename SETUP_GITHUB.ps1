...
param(
  ...
  [string]$ProjectPath = "C:\PowerBI\FFLEXANA",
  [switch]$CreateRemote,
  [switch]$Push
)
$ErrorActionPreference = "Stop"
$RepoName = "FFLEXANA-powerbi-visual"
$ExpectedName = "FFLEXANA"
...
if (-not (git diff --cached --quiet)) { git commit -m "Initial FFLEXANA Power BI visual" }
...
if ($CreateRemote) {
  ...
  if ($LASTEXITCODE -ne 0) { gh repo create "$Owner/$RepoName" --public --description "Independent FFLEXANA Power BI custom visual" --source . --remote origin --push }
...
if ($Push) {
  ...
  if ($PSCmdlet.ShouldProcess("origin/main", "Push FFLEXANA")) { git push -u origin main }
}
Write-Host "Prepared FFLEXANA. Remote creation/push are opt-in switches." -ForegroundColor Green
